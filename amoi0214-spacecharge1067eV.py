...
schist = hist2d(50,0,5,100,0.,1024.)
...
###### --- Offline analysis
ds = psana.DataSource("exp=AMO/amoi0214:run=77,81,82:idx")
for run in ds.runs():
    times = run.times()
...
###### --- End of Offline analysis region
        ...
        #print s*24/1000,len(c)
        # Space Charge histogram
        schist.fill(s*24/1000,len(c))


    if not 'schist_sum_all' in locals():
        schist_sum_all = np.empty_like(schist.data)
    comm.Reduce(schist.data,schist_sum_all)

    if rank==0:

        spectra_name = 'spacecharge/run'+str(args.run).zfill(4)+'_schist'
        np.savetxt(spectra_name,schist_sum_all)
