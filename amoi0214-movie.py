...
###### --- End of Offline analysis region
        ...
        if evtGood%6 == 0:
            ...
            if rank==0:
                ...
                moments_avg_plot = XYPlot(eventCounter*size, "Standart Deviation of X-Projection", ax4, moments_avg_arr,formats=".-") # make a 1D plot
                ...
                hithist_plot = XYPlot(eventCounter*size, "Hit Hist2", ax5, hitxprojhist_sum_all/(len(hitxprojhist_buff)*size),formats=".-") # make a 1D plot
                publish.send("HITHIST", hithist_plot) # send to the display

                # histogram of single shot
                ax6 = range(0,hithist.data.shape[0])
                hit1shot_plot = XYPlot(eventCounter*size, "Hit Hist 1 shot", ax6, hithist.data,formats=".-") # make a 1D plot
                publish.send("HIT1SHOT", hit1shot_plot) # send to the display

                # #
                opal_plot = Image(eventCounter*size, "Opal", opal) # make a 2D plot
